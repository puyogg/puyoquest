...
impl LeaderboardChannelRouter {
    ...
    async fn set_leaderboard_channel(
        ...
        let leaderboard_channel: LeaderboardChannel = sqlx::query_as(
            r#"
                INSERT INTO leaderboard_channel (server_id, game_type, channel_id)
                VALUES ($1, $2, $3)
                ON CONFLICT (server_id, game_type)
                DO UPDATE
                    SET channel_id = $3
                    WHERE leaderboard_channel.server_id = EXCLUDED.server_id
                      AND leaderboard_channel.game_type = EXCLUDED.game_type
                RETURNING *
            "#,
        )
        .bind(&server_id.0)
    ...
    async fn delete_leaderboard_channel(
        ...
        let _ = sqlx::query(
            r#"
                DELETE FROM leaderboard_channel
                WHERE server_id = $1 AND game_type = $2
            "#,
        )
        .bind(&server_id.0)
