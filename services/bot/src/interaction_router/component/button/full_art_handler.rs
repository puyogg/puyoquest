...
pub async fn full_art_handler(
    ...
    let full_art = sdk::apis::cards_api::cards_card_id_full_art_get(&data.api_config, &card_id).await?;

    let (embed, components) = embeds::full_art_embed(&card, &full_art, embeds::FullArtType::NormalLeft);
...
type CardId = String;
pub fn parse_full_art_embed_custom_id(custom_id: &str) -> Result<Option<CardId>, Error> {
...
pub fn full_art_nav_button(card_id: &str) -> serenity::CreateButton {
    let custom_id = format!("full_art:update::{card_id}:");
    serenity::CreateButton::new(custom_id).label("Full Art")
}
