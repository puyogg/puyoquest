...
function calcualteIpv6Cidr(cidrBlock: string, idx: number): string {
  ...
  base10Slices[base10Slices.length - 1] =
    base10Slices[base10Slices.length - 1] + idx;

  const backToHex = base10Slices.map((dec) => dec.toString(16).padStart(4));
  return `${backToHex.join(":")}::/64`;
}

class VpcIpv6 extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this.internetGateway = new aws.ec2.InternetGateway(`${name}-ig`, {
      vpcId: this.vpc.id,
    });
    ...
    this.publicSubnet = new aws.ec2.Subnet(`${name}-public-subnet`, {
      ...
      ipv6CidrBlock: this.vpc.ipv6CidrBlock.apply((cidr) =>
        calcualteIpv6Cidr(cidr, 0)
      ),
    });
